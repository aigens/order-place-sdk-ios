...
class AlipayService: OrderPlaceService {
    ...
    private func payOrder(body: NSDictionary, callback: CallbackHandler?) {
        ...
        // call back 是wap 支付的结果, 钱包支付的结果要在appdelegate 中写

        AlipaySDK.defaultService().payOrder(body.value(forKey: "orderStr") as? String ?? "", fromScheme: appScheme) { (result) in
